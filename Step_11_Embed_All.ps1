...
function Embed-All {
    DrawBanner "STEP 11 EMBED SUBTITLES AND FINALIZE"
    ...
    Get-ChildItem -LiteralPath $TempDir -Recurse -Filter *.mkv | ForEach-Object {
        ...
        if ($embedOK) {
            ...
            $stats.FAILED++
            $embedStatus = "FAILED"
            $failedList.Add([pscustomobject]@{ Name = $mkvBase; Reason = "Embed mislukt (FFmpeg fout)" })
            return
        }
        ...
        try {
            ...
            $stats.MOVED++
            $moveStatus = "OK"
            $successList.Add($cleanMkvBase)
        } catch {
            Show-Format "ERROR" "$mkvName" "Copy failed: $_" -NameColor "Red"
            $stats.FAILED++
            $moveStatus = "FAILED"
            $failedList.Add([pscustomobject]@{ Name = $mkvBase; Reason = "Kopiëren mislukt: $_" })
        }
        DrawBar "*"
    ...
    # ─── Eindverslag ──────────────────────────────────────────────────────
    DrawBanner "EINDVERSLAG"
    Write-Host "  Gelukt  : $($successList.Count)" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "  Mislukt : $($failedList.Count)" -ForegroundColor Red
    if ($failedList.Count -gt 0) {
        foreach ($entry in $failedList) {
            Write-Host "    [FAIL] $($entry.Name)" -ForegroundColor Red
            Write-Host "           Reden: $($entry.Reason)" -ForegroundColor DarkRed
        }
    }
    DrawBar "="
}
...
function Start-Embed {
    Start-StepLog -StepNumber "11" -StepName "Embed_All"
    Embed-All
    Stop-StepLog
}
