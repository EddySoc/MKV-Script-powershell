...
if ($Global:ConfigLoaded) {
    try {
        ...
        if (Get-Command Stop-LingeringSTTProcesses -ErrorAction SilentlyContinue) {
            Stop-LingeringSTTProcesses -WhisperExe $Global:STTExe -KeepPids $Global:InitialSTTPids
        }
    }
} else {
