...
function Clean-AllSubtitles {
    ...
    if ($allSubs.Count -eq 0) {
        ...
        Set-StepRunResult -Step "08" -Success 0 -Failed 0 -FailedItems @() -Note "no subtitles found"
        return
    }
    ...
    Set-StepRunResult -Step "08" -Success $cleanedCount -Failed 0 -FailedItems @() -Note "skipped=$skippedCount"
}
...
function Start-CleanSubs {
    ...
    if ($runClean -eq $false -or $runClean -eq "false") {
        ...
        Set-StepRunResult -Step "08" -Success 0 -Failed 0 -FailedItems @() -Note "step skipped (RunClean=false)"
        Stop-StepLog
        return
    }
    Clean-AllSubtitles
